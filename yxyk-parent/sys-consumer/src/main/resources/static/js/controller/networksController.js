app.controller("networksController",function ($scope,$location,networksService,$filter) {
    ...
    $scope.findReportModulars=function(){
        networksService.findReportModulars().success(
            function (respone) {
    ...
    $scope.voCondition.transactionState=0;
    ...
    $scope.findall=function () {
        networksService.findall($scope.voCondition).success(function (response) {
    ...
    $scope.changem=function(reportId){
        ...
        networksService.updateModular($scope.voUpdateModular).success(
            function (respone) {
                $scope.reloadlist()
            }
        )
    ...
    $scope.addInspectList=function (reportId,transactionState) {
        $scope.voAddInspectList.reportIds.push(reportId);
        if(transactionState==3){
            $scope.voAddInspectList.transactionState=2;
            $scope.addsInspectList(2)
        }else {
            $scope.addsInspectList(transactionState)
        }
        window.location.href="/PopularOpinion/departmentDetails?id="+reportId;
    };
    $scope.detail=function(reportId,readState){
        if(readState==1){
            networksService.saveIsRead(reportId).success(
            )
        }
        window.location.href="/PopularOpinion/networklistDetails?id="+reportId
    }
    //单个  加入核查
    $scope.addsInspectList=function (reportId,transactionState) {
            ...
            BootstrapDialog.show({
                ...
                buttons: [{
                    ...
                    action: function(dialogItself) {
                       adds(transactionState,reportId);
                        dialogItself.close();
                    }
                }, {
                    label: '取消',
    ...
    //多个 加入核查
    $scope.InspectList=function (transactionState) {
    ...
    function add(transactionState){
        $scope.voAddInspectList.transactionState=transactionState;
        var lists =$scope.report.valueOf();
        for(var i in lists){
            if (lists[i]){
                $scope.voAddInspectList.reportIds.push(i);
            }
        }
        networksService.addInspectList($scope.voAddInspectList).success(function () {
                $scope.voAddInspectList.reportIds=[];
                $scope.reloadlist();
            }
        )
    }
    function adds(transactionState,reportId){
        $scope.voAddInspectList.transactionState=transactionState;
        $scope.voAddInspectList.reportIds.push(reportId);
        networksService.addInspectList($scope.voAddInspectList).success(function () {
                $scope.voAddInspectList.reportIds=[];
        ...
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                action: function(dialogItself) {
                    ...
                    networksService.delete($scope.reportIds).success(
                        function (response) {
                            $scope.reportIds=[];
                            $scope.reloadlist();
                        }
